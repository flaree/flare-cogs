...
class Stats:
    ...
    async def profileid(self, name, platform):
        async with self._session.get(
            self.url + "/getSmallUser.php?name={}&platform={}&appcode=flare".format(name, platform)
        ) as response:
    ...
    async def profile(self, account, platform):
        async with self._session.get(
            self.url + "/getUser.php?name={}&platform={}&appcode=flare".format(account, platform)
        ) as response:
    ...
    async def season(self, account, platform, season):
        async with self._session.get(
            self.url
            + "/getUser.php?name={}&platform={}&season={}&appcode=flare".format(
                account, platform, season
            )
        ) as response:
            return await response.json(content_type="text/html")
    ...
    async def stats(self, account, platform):
        async with self._session.get(
            self.url + "/getStats.php?name={}&platform={}&appcode=flare".format(account, platform)
        ) as response:
            return await response.json(content_type="text/html")
    ...
    async def operators(self, account, platform):
        async with self._session.get(
            self.url
            + "/getOperators.php?name={}&platform={}&appcode=flare".format(account, platform)
        ) as response:
            return await response.json(content_type="text/html")
