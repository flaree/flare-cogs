...
class Wcrp(BaseCog):
    ...
    async def ip(self, ctx):
        ...
        embed = discord.Embed(title="West Coast Roleplay Information", colour=discord.Colour(value=colour))
        ...
        embed.add_field(name="Discord Invite Link", value="https://discord.gg/2jZYuZ7", inline=True)
    ...
    async def vpncheck(self, ctx, ip: str):
        ...
        vpn1 = "http://proxycheck.io/v2/{}?key=6n5t9m-353305-68f6j3-1qfw04&vpn=1&asn=1&node=1&time=1&inf=0&port=1&seen=1&days=7&tag=msg".format(
            ip)
        vpn2 = "http://check.getipintel.net/check.php?ip={}&contact=flare2399@gmail.com&format=json&flags=m".format(ip)
        r = await self.get(vpn1)
        t = await self.get(vpn2)
        user = ctx.author
        colour = randint(0, 0xFFFFFF)  # Random Hex Value for embed colour.
        embed = discord.Embed(title=f"VPN Checker for {ip}",
                              colour=discord.Colour(value=colour), timestamp=ctx.message.created_at)
        try:
            embed.add_field(name="Proxy Status(Test One):", value=r[f'{ip}']['proxy'].capitalize(), inline=True)
            if r[f'{ip}']['proxy'] == "yes":
            ...
            if t['result'] == "1":
                ...
                embed.add_field(name="Proxy Status(Test Two):", value="No".capitalize(), inline=True)
            embed.set_footer(
                text="VPN information requested by " + str(user))
            await ctx.send(embed=embed)
            message = ctx.message
            await message.add_reaction("\N{WHITE HEAVY CHECK MARK}")
        except KeyError:
    ...
    async def rules(self, ctx):
        ...
        embed = discord.Embed(title="West Coast RP Rules", colour=0xff6a14)
        embed.set_thumbnail(url="http://flareee.com/i/c6duh.png")
        embed.add_field(name="Information", value="Welcome to the Discord server of **West Coast Role Play**!")
        embed.add_field(name="Rules",
                        value="**1**. Absolutely NO racism. \n**2**. Keep discussions to relevant text channels.\n**3**. Don't scream / play annoying distorted audio clips in voice channels.\n**4**. Don't play music in voice channels.\n**5**. No spamming\n**6**. English only\n**7**. If you see anyone breaking these rules, please take a screenshot and send it to a moderator\n**8**. No links or images in main chat\n\n\n",
                        inline=True)
        await ctx.send(embed=embed)
    ...
    async def lspd(self, ctx):
        embed = discord.Embed(title="WC-RP | Los Santos Police Department", colour=0xff6a14,
        ...
        embed.set_thumbnail(url="https://vignette.wikia.nocookie.net/lsrp/images/b/bb/Seal_of_the_LSPD.png")
    ...
    async def lsfd(self, ctx):
        embed = discord.Embed(title="WC-RP | Los Santos Fire Department", colour=0xff6a14,
    ...
    @commands.command(pass_context=True, no_pm=True)
    @commands.has_any_role("Management", "Senior Administrator", "Server Owner", "Server Manager", "Scripter",
                           "Lead Administrator", "Administrator")
    ...
    @commands.command()
    @commands.has_any_role("Management", "Senior Administrator", "Server Owner", "Server Manager", "Scripter",
                           "Lead Administrator", "Faction Administrator")
